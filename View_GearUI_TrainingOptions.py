...
def start(sender, app_data, user_data):
    ...
    with dpg.window(label=f" Training Options For {gear}", tag=window_tag, width=430, height=558,
                    on_close=lambda: (dpg.show_item(home_ui_window), dpg.delete_item(window_tag)), no_move=True):
        ...
        with dpg.group(horizontal=True):
                dpg.add_spacer(width=padding)
                dpg.add_button(label="Train",tag = "train_button", callback=View_GearUI_TrainingOptions_Train.show_timer, width=button_width, height=100, user_data=gear)
